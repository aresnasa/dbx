// PATCHED for RusTerm integration: dbx-web was a [[bin]]-only crate. This
// lib.rs extracts the router/state builders (build_app_router,
// build_web_state, build_app_state, run_server, ServerConfig) so RusTerm can
// mount the full dbx web UI under /dbx in its relay HTTP server. main.rs is
// now a thin wrapper. Regenerate via scripts/setup-dbx.sh (see
// third_party/PATCHES.md #6).
//! dbx-web — reusable library form of the DBX web server.
//!
//! The `dbx-web` *binary* is a thin wrapper ([`run_server`]) over the pieces here,
//! which can be embedded into a host application (e.g. RusTerm mounts the router
//! built by [`build_app_router`] under `/dbx` inside its own axum server).
//!
//! Layout:
//! - [`build_app_state`] — storage + dialects + plugins + agents (dbx-core `AppState`).
//! - [`build_web_state`] — per-server [`WebState`] (auth, SSE channels, ssh prompts …).
//! - [`build_app_router`] — the full route table + middleware + static frontend.
//! - [`run_server`] — standalone bind/serve loop (what the binary runs).

// dbx-web is now an embeddable library: its route handlers are meant to be
// assembled into a Router (see build_app_router) rather than referenced as
// concrete types downstream. Several handlers expose pub(crate) request/response
// structs through pub fn signatures; silence that lint at the crate root so the
// embedded build stays quiet without churning the visibility of dozens of types.
#![allow(private_interfaces)]
// The database-export route spawns a future whose Send derivation walks a
// very deep combinator chain (BufferUnordered(FuturesUnordered(...)) into
// dbx-core's get_table_ddl_core_with_options), exceeding rustc's default
// recursion limit of 128. That used to be silently accepted and is now the
// future-incompat `recursion_depth_exceeding_limit` warning (rust#159228);
// raise the limit so the auto-trait evaluation completes instead.
#![recursion_limit = "512"]
...
pub fn web_compression_predicate() -> impl Predicate {
...
pub fn web_body_limit_bytes() -> usize {
...
pub fn web_agent_dir(data_dir: &std::path::Path) -> std::path::PathBuf {
    web_agent_dir_from_env(data_dir, std::env::var("DBX_AGENT_DIR").ok())
}

pub fn web_agent_dir_from_env(data_dir: &std::path::Path, agent_dir: Option<String>) -> std::path::PathBuf {
    agent_dir.map(std::path::PathBuf::from).unwrap_or_else(|| data_dir.join("agents"))
}

pub fn normalize_public_base_path(value: Option<String>) -> String {
    let trimmed = value
...
pub fn add_mq_routes(router: Router<Arc<WebState>>) -> Router<Arc<WebState>> {
    router
...
/// Standalone-server configuration (see [`run_server`]).
#[derive(Debug, Clone, Default)]
pub struct ServerConfig {
    /// Data directory (storage, plugins, agents). `None` → `$DBX_DATA_DIR` or `~/.dbx-web`.
    pub data_dir: Option<std::path::PathBuf>,
    /// TCP port. `None` → `$DBX_PORT` or 4224.
    pub port: Option<u16>,
    /// Static frontend asset directory. `None` → `$DBX_STATIC_DIR` or none.
    pub static_dir: Option<String>,
    /// Public base path (e.g. `"/dbx"`). `None` → `$DBX_PUBLIC_BASE_PATH` or `/`.
    pub public_base_path: Option<String>,
    /// Disable password auth entirely (trusted/local embedding).
    pub password_disabled: bool,
    /// Explicit password (hashed at startup). `None` → `$DBX_PASSWORD` or stored hash.
    pub password: Option<String>,
}

impl ServerConfig {
    /// Build a config from process env vars (the binary behaviour).
    pub fn from_env() -> Self {
        let password_disabled = std::env::var("DBX_DISABLE_PASSWORD")
            .map(|v| matches!(v.trim().to_lowercase().as_str(), "1" | "true" | "yes" | "on"))
            .unwrap_or(false);
        Self {
            data_dir: std::env::var("DBX_DATA_DIR").ok().map(std::path::PathBuf::from),
            port: std::env::var("DBX_PORT").ok().and_then(|p| p.parse().ok()),
            static_dir: std::env::var("DBX_STATIC_DIR").ok(),
            public_base_path: std::env::var("DBX_PUBLIC_BASE_PATH").ok(),
            password_disabled,
            password: std::env::var("DBX_PASSWORD").ok(),
        }
    }
}

/// Build the dbx-core [`AppState`](dbx_core::connection::AppState): opens storage,
/// registers dialects, loads dialect plugins, starts the hot-reload watcher.
pub async fn build_app_state(
    ...
    std::sync::Arc::new(AppState::new_with_plugin_and_agent_dir_and_app_version(
        storage,
...
/// Build the per-server [`WebState`] from a dbx-core `AppState`.
///
/// Password policy: `cfg.password_disabled` wins; otherwise `cfg.password`;
/// otherwise `$DBX_PASSWORD`; otherwise the hash persisted in storage.
pub async fn build_web_state(
    app_state: std::sync::Arc<AppState>,
    data_dir: std::path::PathBuf,
    cfg: &ServerConfig,
) -> std::sync::Arc<WebState> {
    let password_disabled = cfg.password_disabled;
    ...
    let public_base_path = match &cfg.public_base_path {
        Some(p) => normalize_public_base_path(Some(p.clone())),
        None => normalize_public_base_path(std::env::var("DBX_PUBLIC_BASE_PATH").ok()),
    };

    let web_state = std::sync::Arc::new(WebState {
        app: app_state,
        data_dir,
        ...
        ssh_prompts: std::sync::Arc::new(ssh_prompt::SshPromptHub::new()),
    });
    ssh_prompt::install_web_ssh_prompt_bridge(web_state.ssh_prompts.clone());
    web_state
}

/// Assemble the full axum `Router`: API routes, auth middleware, body-limit /
/// compression / trace layers, optional static frontend fallback, and public
/// base-path wrapping. Ready to serve directly or nest into a host router
/// (e.g. RusTerm mounts this under `/dbx`).
pub fn build_app_router(web_state: std::sync::Arc<WebState>, static_dir: Option<String>) -> axum::Router {
    let api = Router::new()
        // Auth
    ...
    let public_base_path = web_state.public_base_path.clone();
...
/// Resolve config, build state + router, bind and serve until Ctrl-C.
/// This is what the `dbx-web` binary runs.
pub async fn run_server(cfg: ServerConfig) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    rustls::crypto::aws_lc_rs::default_provider().install_default().expect("Failed to install rustls crypto provider");

    let data_dir = cfg.data_dir.clone().unwrap_or_else(|| {
        std::env::var("DBX_DATA_DIR").map(std::path::PathBuf::from).unwrap_or_else(|_| {
            let home = std::env::var("HOME").unwrap_or_else(|_| ".".to_string());
            std::path::PathBuf::from(home).join(".dbx-web")
        })
    });
    std::fs::create_dir_all(&data_dir).expect("Failed to create data directory");
    let agent_dir = web_agent_dir(&data_dir);
    let app_state = build_app_state(&data_dir, &agent_dir, env!("CARGO_PKG_VERSION")).await;
    let web_state = build_web_state(app_state, data_dir, &cfg).await;
    let app = build_app_router(web_state.clone(), cfg.static_dir.clone());

    let port = cfg.port.unwrap_or_else(|| std::env::var("DBX_PORT").ok().and_then(|p| p.parse().ok()).unwrap_or(4224));
    let addr = SocketAddr::from(([0, 0, 0, 0], port));
    tracing::info!("DBX Web server starting on http://{}", addr);
    if web_state.public_base_path != "/" {
        tracing::info!("Serving DBX Web under context path {}", web_state.public_base_path);
    }
    if web_state.password_disabled {
